...
apa.cor.table(dat_w1 %>% 
                ...
                select(
                  ...
                  "prqc_overall", "prqc_satisfaction", "prqc_commitment",
                  "prqc_intimacy", "prqc_trust", "prqc_passion", "prqc_love"
                ),
...
```{r plots-prqc}
for(ivar in paste0(c("Overall", "Satisfaction", "Commitment",
                     "Intimacy", "Trust", "Passion", "Love"))){
  p <- plot_hist(var = paste0("prqc_", tolower(ivar)), 
                 var_name = paste("PRQC Relationship Quality -", ivar), 
                 data = dat_w1, bin_width = 0.5,
                 var_sex = "sex", female_male = c("1", "2")) +
    xlim(1,7)

  if(signif(var = paste0("prqc_", tolower(ivar)), 
            var_sex = "sex",
            data = dat_w1,
            female_male = c("1", "2"))) {
    p <- p + labs(subtitle = "Significant sex difference (p < 0.05)")
