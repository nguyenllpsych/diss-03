...
for (var in unique(prqc$subscale)) {
  ...
  # create and append scale scores
  assign(x = var, value = rowMeans(prqc_df, na.rm = T))
  dat_w1[var] <- get(var)
  assign(x = var, value = rowMeans(prqc_df_w2, na.rm = T))
  dat_w2[var] <- get(var)
  assign(x = var, value = rowMeans(prqc_df_w3, na.rm = T))
  dat_w3[var] <- get(var)
}
```
...
dat_w1 <- dat_w1 %>%
  select(all_of(c(basics$var_new, bfas_self$subscale, bfas_partner$subscale,
                  aaq$subscale, "resp", "trust", prqc$subscale)))
dat_w2 <- dat_w2 %>%
  select(all_of(c(basics$var_new[1], bfas_fu$subscale, bfas_partner$subscale,
                  aaq$subscale, "resp", "trust", prqc$subscale)))
dat_w3 <- dat_w3 %>%
  select(all_of(c(basics$var_new[1], bfas_fu$subscale, bfas_partner$subscale,
                  aaq$subscale, "resp", "trust", prqc$subscale)))
